...
class Student
{
    ...
    {
        if (!$this->_db->insert('em_students', $fields)) {
    ...
    public function update($fields = array(), $fac_id = null)
    {
        ...
        if (!$this->_db->update('em_students', $fac_id, $fields)) {
    ...
    {        
        if ($user) {
            ...
            $data = $this->_db->get('em_students', array('fac_username', '=', $user));
            if ($data->count()) {
                $this->_data = $data->first();
                return true;
            } else {
                $datas = $this->_db->get('em_students', array('fac_email', '=', $user));
                if ($datas->count()) {
                    $this->_data = $datas->first();
                    return true;
                } else {
                    $datap = $this->_db->get('em_students', array('fac_passkey', '=', $user));
                    if ($datap->count()) {
    ...
    public function find_by_hash($user = null)
    {
        if ($user) {
            ...
            $data = $this->_db->get('em_students', array($field, '=', $user));
