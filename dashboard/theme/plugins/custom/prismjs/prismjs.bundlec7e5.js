			...
			a = {
				...
				highlightElement: function (n, r, i) {
					...
					o &&
						...
						(o.className = o.className.replace(t, '').replace(/\s+/g, ' ') + ' language-' + s);
					...
					if ((a.hooks.run('before-sanity-check', u), !u.code))
						return a.hooks.run('complete', u), void (i && i.call(u.element));
			...
			for (var g in n)
				if (n.hasOwnProperty(g) && n[g]) {
					...
					for (var p = 0; p < d.length; ++p) {
						...
						for (
							var F = m.pattern || m, k = s.next, w = u;
							...
							if (!(A instanceof r)) {
								...
								if (v && k != t.tail.prev) {
									...
									var $ = E.index + (h && E[1] ? E[1].length : 0),
										...
										_ = w;
									for (_ += k.value.length; $ >= _; ) _ += (k = k.next).value.length;
								...
								if (E) {
									...
									var O = k.prev;
									z && ((O = l(t, O, z)), (w += z.length)),
										o(t, O, P),
										(k = l(t, O, new r(g, f ? a.tokenize(C, f) : C, b, C))),
										T && l(t, k, T),
										P > 1 && i(e, t, n, k.prev, w, { cause: g + ',' + p, reach: N });
								}
							}
						}
					}
										...
										: (l.setAttribute(n, 'failed'),
										  m.status >= 400
												? (u.textContent =
														((t = m.status),
														(a = m.statusText),
														'âœ– Error ' + t + ' while fetching file: ' + a))
												: (u.textContent = 'âœ– Error: File does not exist or is empty')));
							}),
							m.send(null);
					}
				}),
							...
							if (e.code && n && 'pre' === n.nodeName.toLowerCase()) {
								for (var a = n.childNodes, r = '', i = '', s = !1, l = 0; l < a.length; ++l) {
									...
									o == e.element
										? (s = !0)
										: '#text' === o.nodeName &&
										  (s ? (i += o.nodeValue) : (r += o.nodeValue), n.removeChild(o), --l);
								}
