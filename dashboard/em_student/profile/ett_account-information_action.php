...
if (Input::exists()) {
    ...
    $ett_teacher = new Student(convert_string('encrypt', Input::get('ett_token')));
    ...
    if (!$ett_teacher->exists()) {
        ...
        if (Input::get('action') == "Teacher_update") {
            ...
            if (Input::get('te_username') == $data->fac_username) {
                ...
                $query = "SELECT fac_username FROM em_students WHERE fac_username = :et_username";
                ...
                if ($row > 0) {
                    ...
                    $query = "UPDATE em_students SET fac_username = :et_username, fac_updated_at = :ats WHERE fac_passkey = :et_token";
                    $statement = $connect->prepare($query);
                    $statement->execute(
                        array(
                            ':et_username' => Input::get('te_username'),
            ...
            if (Input::get('te_email') == $data->fac_email) {
                ...
                $query = "SELECT fac_email FROM em_students WHERE fac_email = :et_email";
                $statement = $connect->prepare($query);
                $statement->execute(
                    array(
                ...
                if ($row > 0) {
                    ...
                    $query = "UPDATE em_students SET fac_email = :et_email WHERE fac_passkey = :et_token";
                    $statement = $connect->prepare($query);
                    $statement->execute(
                        array(
                            ':et_email' => Input::get('te_email'),
            ...
            if (Input::get('te_pass')) {
                if (convert_string('encrypt', Input::get('te_pass')) != $data->fac_password) {
                    $query = "UPDATE em_students SET fac_password = :et_password WHERE fac_passkey = :et_token";
                    $statement = $connect->prepare($query);
                    $statement->execute(
                        array(
                    ...
                    if (isset($result)) {
                        ...
                        $error .= "Error! While Updating password<br>";
                    }
                } else {
            ...
            if(Input::get('ett_year') != $data->fac_year){                
                $query = "UPDATE em_students SET fac_year = :roles, fac_updated_at = :ett_updated_at WHERE fac_passkey = :ett_token";
                $statement = $connect->prepare($query);
                if($statement->execute(array(
                    ':roles' => Input::get('ett_year'),
                    ':ett_updated_at' => $te_updated_at,
                    ':ett_token' => convert_string('encrypt',Input::get('ett_token'))
                ))){
                    $message .= "Student Year Updated<br>";
                }
            ...
            if(Input::get('te_dept') != convert_string('decrypt',$data->fac_dept_hash)){
                ...
                if($da->exists()){
                    ...
                    $query = "UPDATE em_students SET fac_department = :dept, fac_dept_hash = :dept_hash, fac_updated_at = :ett_updated_at WHERE fac_passkey = :ett_token";
                    $statement = $connect->prepare($query);
                    if($statement->execute(array(
                        ':dept' => $das->dept_name,
                        ...
                        $message .= "Student Department Updated<br>";
                    }
                    
                }
            }

            if(Input::get('ett_mentor') != convert_string('decrypt',$data->fac_mentor_hash)){
                $daf = new Faculty(convert_string('encrypt',Input::get('ett_mentor')));
                if($daf->exists()){
                    $dar = $daf->data();

                    $query = "UPDATE em_students SET fac_mentor = :dept, fac_mentor_hash = :dept_hash, fac_updated_at = :ett_updated_at WHERE fac_passkey = :ett_token";
                    $statement = $connect->prepare($query);
                    if($statement->execute(array(
                        ':dept' => $dar->fac_name,
                        ':dept_hash' => $dar->fac_passkey,
                        ':ett_updated_at' => $te_updated_at,
                        ':ett_token' => convert_string('encrypt',Input::get('ett_token'))
                    ))){
                        $message .= "Student Mentor Updated<br>";
                    }
        ...
        if (Input::get('action') == "status") {
            if (Input::get('ett_id') == "deactivate_account") {
                $query = "UPDATE em_students SET fac_status = :et_status WHERE fac_passkey = :et_value";
                $statement = $connect->prepare($query);
                $statement->execute(
                    array(
                        ':et_status' => "Inactive",
